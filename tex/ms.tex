...
calibration vector the night constructed from.... (MORE HERE). Figure
\ref{fig:data} shows the spectra before and after calibration for each
cluster.  Discuss S/N, sky subtraction.
...
parameters $\theta^\prime$ include the stellar mass $m_*$, effective optical
depth to dust in the $V$-band $\tau_V$, luminosity distance $d_L$,
and redshift $z$.  The obsrved frame wavelengths $\lambda_o$ are
related to the restframe wavelengths $\lambda$ in the usual way.
...
BDJ Discuss broadening here.  The instrumental resolution is given by $\sigma$,
...
\mu(\theta, \lambda_o) &  = & (G_\sigma  \otimes F_{\theta^\prime})(\lambda) +
...
\quad ,
\end{eqnarray}
where $G$ represents a Gaussian of unit area, $\otimes$ indicates
convolution, and the full parameter set $\theta$ now includes the
amplitudes of the $N_l$ nebular and interstellar emission lines
$A_\ell$.  We again implicitly include the conversion from restframe
to observed frame spectra.
