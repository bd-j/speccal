...
XXX Alternative XXX.
Because the model  of equation \ref{eq:spectroscopicLF} includes an
additive term in the form of the residual column vector $\Delta$,
sharp spectral features can be diluted. To mitigate this we try a
model where the residual column vector is taken to be the residual
\emph{from the calibration vector $f(\alpha)$}.  That is, 
\begin{eqnarray}\label{eq:spectroscopicLFalt}
\Delta &\equiv& \frac{d}{\mu(\theta)} - f(\alpha)
\quad ,
\end{eqnarray}
