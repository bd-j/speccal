...
subtraction happened. The data were approximately
spectrophotometrically calibrated using an average calibration vector
the night constructed from.... (MORE HERE). Figure \ref{fig:data}
shows the spectra before and after calibration for each cluster.
Discuss S/N, sky subtraction.
...
& & \sum_{i=0}^{N_l} \frac{a_i}{\sigma_e\sqrt{2\pi}} \exp(-\frac{(\lambda - \lambda_i)^2}{2\sigma_e^2})
\nonumber \\ 
\lambda_o & = & (1+z)\,\lambda 
\\
