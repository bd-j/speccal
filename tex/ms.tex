...
using Voight profiles for the line shapes.  NB GAUSSIANS ARE A NOT
GREAT APPROXIMATION FOR ABSORPTION LINE SHAPES (GAUSSIAN IN OPTICAL
DEPTH, NOT $\Delta F$)
...
the spectral calibration and infer it along with the physical
...
incorporated into informative priors on the calibration parameters of
our model.
...
the spectrscopy and the photometry are sampling the same populations.
In practice, differences between spectroscopic and photometric
apertures mean that the photometry and the spectroscopy are sampling
