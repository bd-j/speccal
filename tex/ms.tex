...
calibration {\color{blue} SHOULD THIS SHOW ALL THE CLUSTERS OR JUST ONE?.  Discuss
S/N, sky subtraction.}

{\color{red} Specify order of calibration polynomial here?}
...
components of the model constructed from each draw.  This figure also
shows the calibrated spectrum on which the priors were conditioned,
and residuals between the posterior models and the observed data.  In
Figure \ref{fig:inferred_sed} we also show the residuals between the
data and the draws from the posterior for the spectra. In Figure
\ref{fig:inferred_params} we show the marginalized and joint PDFs for
a subset of the full model parameters.
