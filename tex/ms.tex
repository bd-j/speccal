...
as a fractional error, $(s\mu(\theta))^2$, corresponding for example
...
\ln p(d\given\theta,s^2) &=& -\frac{1}{2}\,\sum_{n=1}^N
                             ...
                             {[\sigma_n^2 + (s\,\mu_n(\theta))^2]} + 
                             \ln(2\pi\,[\sigma_n^2 + (s\,\mu(\theta))^2]) \right]
\quad .
\end{eqnarray}
...
In Figure \ref{fig:speconly_calibrated} we show the result of our
inference from the spectrum and a single photometric measurement (SDSS
$g$) for a perfectly calibrated mock spectrum. To demonstrate the
quality of the fit we show both the mock
input spectrum and predictions of the observed spectra that result
from several draws of the model parameters
$[\theta, \alpha]$ from posterior distribution.  We also show samples
of the inferred calibration vector,
\begin{eqnarray} 
...
Even in the case of a single photometric data point, several of the
...
though hewre we have propogated the uncertainty in this normalization
