...
redshift. The instrumental resolution is given by $\sigma$.  Discuss
Intrinsic resolution of model spectra (MILES), whether instrumentasl
resolution is larger, smaller, or comparable to intrinsic velocity
broadening.
...
Figure \ref{fig:inferred_spectrum} we show the spectrum resulting
...
\ref{fig:inferred_spectrum} we also show the residuals between the data
and the draws from the posterior, for both the spectra and the
photometry. In Figure \ref{fig:inferred_params} we show the
...
\caption{Show, for one cluster, the (un)calibrated spectral data, draws from
  the posterior for the model, and residuals \label{fig:inferred_spectrum}}
\end{figure}

\begin{figure}[h!]
\caption{Show the observed photometric SED and draws from the
  posterior, for one cluster.\label{fig:inferred_sed}}
\end{figure}

\begin{figure}[h!]
\caption{Show a corner plot of selected relevant parameters, for one
cluster. \label{fig:inferred_params}}
\end{figure}

\begin{figure}[h!]
\caption{Show the calibration vector and our inference of it? \label{fig:inferred_calib}}
\end{figure}
...
\caption{Money plot.  Show the difference between the parameters
  ...
  cluster.\label{fig:boom}}
\end{figure}

\section{Discussion}

BDJ INSERT WORDS HERE.  We have demonstrated that we can combine
photometric and spectroscopic data in a principled way to infer
physical properties.  We do not resort to an arbitrary down-weighting
or up-weighting of certain data. Instead, we explicitly model our
uncertain knowledge of the spectral calibration.  For a number of
...
if it exists, can be used to generate informative priors on the
calibration parameters of our model. Furthermore, our strategy is
easily extensible to more complicated models, which include for
example night sky line emission in the spectroscopy, wavelength
calibration.  

In a heirarchical model it should be possible to use the inferred
calibration parameters, along with additional information about the
observations (e.g. airmass, instrument temperature), to in turn infer
the parameters of a generative model for the calibration.

Limitations.  The strategy we have outlined here has several
limitations.  First, it requires a generative model for the spectrum of
the object of interest, and is thus ill-suited to discovery projects.

BDJ misc issues, collected here in case we want to address them and
