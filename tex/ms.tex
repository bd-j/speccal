...
term.  Large $s$ reduces the penalty for individual large residuals,
...
the GP {\color{blue} BDJ make this more precise and correct} only
reduces the likelihood penalty for residuals on the scale of $\ell$
with amplitude $a$ that {\it can't} be fit by the mean model.
...
difference is that the inferred calibration vector now contains numerous small scale wiggles,
similar to the actual calibration vector.  The overall shape of the
spectrum is still unconstrained, leading to large degeneracies between
the polynomial terms and the dust attenuation.
