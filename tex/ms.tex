...
velocity dispersion $\sigma_v$, luminosity distance $d_L$ and redshift
$z$.  The spectrum of one solar mass of stars that formed at time $t$
with metallicity $Z$ is given by $S(\lambda, t, Z [, \beta])$ where
the additional parameters $\beta$ may include details of the
...
precision from independent methods.
...
intial round of minimization.  \texttt{emcee} is an implementation of
the affine-invariant ensemble sampler of \citet{GW}, and unlike
typical Metropolis-Hastings MCMC sampling does not require the
specification of step sizes in each dimension.  Our prior
distributions are in all cases top-hat functions, though more
